...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.multi_attn = nn.MultiheadAttention(embed_dim=embed_size, num_heads=num_heads,
                                                dropout=dropout)
    ...
    def forward(self,
                ...
                key: torch.Tensor,
                value: torch.Tensor,
                key_value_mask: torch.ByteTensor = None,
                attention_mask: torch.ByteTensor = None):
        """
        Parameters
        ----------
        ...
        if key_value_mask is not None:
            key_padding_mask = (1 - key_value_mask).bool()
        else:
            key_padding_mask = None

        # shape : (query_sequence_length, batch_size, feature_size)
        output, _ = self.multi_attn(query.transpose(0, 1), key.transpose(0, 1), value.transpose(0, 1),
                                    key_padding_mask=key_padding_mask, attn_mask=additive_attn_mask)
