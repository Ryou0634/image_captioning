...
from models.submodules.transformer_submodules import (
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.sublayer = clone_modules(SublayerConnection(size, dropout, pre_norm=pre_norm), 3)
...
class TransformerDecoder(SequenceDecoder):
    ...
    def __init__(
        ...
        layer = self._get_layer(size, num_attention_heads, dropout, feedforward_hidden_dim, activation, pre_norm)
    ...
    def time_batch_forward(
        ...
        outputs = self._forward(
            embedding_sequence, mask=mask, state=state, apply_target_subsequent_mask=apply_target_subsequent_mask
        )
